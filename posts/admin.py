...
from import_export.admin import ExportMixin # Requiere: pip install django-import-export
...
class LogResource(resources.ModelResource):
    ...
    evento_nombre = fields.Field(
        ...
        attribute='evento__nombre'
    )
    evento_tipo = fields.Field(
        column_name='Tipo de Acción',
        attribute='evento__tipo'
    )
    evento_modulo = fields.Field(
        column_name='Módulo del Sistema',
        attribute='evento__modulo'
    )
    ...
    class Meta:
        ...
        fields = (
            'fecha_hora', 'user', 'email', 'ci', 
            'evento_modulo', 'evento_nombre', 'evento_tipo', 
            'resultado', 'direccion_ip', 'detalle'
        )
        export_order = fields
...
class CustomUserAdmin(UserAdmin):
    model = User
    list_display = ['username', 'email', 'nombre_completo', 'cargo', 'is_staff', 'is_active']
    list_filter = ['is_staff', 'is_active', 'departamento', 'sucursal']
    
    fieldsets = UserAdmin.fieldsets + (
        ('Información Extra', {'fields': ('nombre_completo', 'ci', 'cargo', 'departamento', 'sucursal')}),
    )
...
class LogAdmin(ExportMixin, admin.ModelAdmin):
    ...
    list_display = ('fecha_hora', 'user', 'evento', 'resultado', 'direccion_ip')
    # Filtros laterales rápidos
    list_filter = ('resultado', 'evento__modulo', 'fecha_hora')
    # Buscador por texto
    search_fields = ('user', 'email', 'ci', 'detalle')
